...
use poem::{handler, http::StatusCode, web::{ self, Data, Json, Path }, Response };
use sea_orm::prelude::Uuid;
use serde::{Deserialize, Serialize};
use tokio::sync::{ RwLock, RwLockWriteGuard};
use std::{ ops::Deref, sync::Arc };
use crate::{ 
    AppState, Rooms, adapters::gateway::sessions::User, domain::{accounts::AccountsService, game::rooms::{self, Partial, Room}}, runtime_storage::Table
};
...
async fn prelude<'a>( //функція для валідації токена, пошук акаунта за токеном та бронювання показників 
    ...
    let player_id = state.accounts.read_account(user.account_id).await
...
pub async fn create(
    ...
    let (mut players, mut rooms, mut player) =
        prelude(state.as_ref(), user, players_ptr.deref(), rooms_ptr.deref()).await?; //виклик функції валідації токена та бронювання показників
